...
export  default function Index({works:serverWorks, materials:serverMaterials, costsize:serverCostSize, news:serverNews, newsitem:serverNewsItem, newsvideo:serverNewsVideo}) {
   ...
   return (
    <Layout title={'Сивек Ватер Джет | Гидроабразивная резка в Москве и Москвовской области'} description={'Гидроабразивная резка, резка водой в Москве и Москвовской области. Доставка по России '} keywords={'Резка водой, Водорезка, гидрорез, гидрорезка, гидроабразив'}>
    <Head>
...
export async function getServerSideProps({req}) {
    ...
    const resworks = await fetch('http://90.156.202.71:7000/works/main')
    const works = await resworks.json();

    const resmat = await fetch('http://90.156.202.71:7000/materials/all')
    const materials = await resmat.json();

    const rescost = await fetch('http://90.156.202.71:7000/costsize/all')
    const costsize = await rescost.json();

    const resnews = await fetch('http://90.156.202.71:7000/news/main')
    const news = await resnews.json();

    const resitem = await fetch('http://90.156.202.71:7000/news/main/item')
    const newsitem = await resitem.json();

    const resvideo = await fetch('http://90.156.202.71:7000/news/main/video')
    const newsvideo = await resvideo.json();
