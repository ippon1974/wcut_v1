...
const Work = ({work:serverWork, prev:serverPrev, next:serverNext, maxid:serverMaxId, minid:serverMinId}) => {
    ...
    useEffect(()=>{
       async function load() {
        ...
        const response = await fetch(`http://23.105.246.179:3000/api/works/next/${router.query.id}`);
        const data = await response.json();
        if(data[0] == undefined){
            return
        }
...
export async function getServerSideProps({query, req}) {
    ...
    const respnext = await fetch(`http://23.105.246.179:3000/api/works/next/${query.id}`);
