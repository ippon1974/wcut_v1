...
const Size = ({mname:serverMName, costsize:serverCostSize, costsizelist:serverCostSizeList}) => {
    ...
    useEffect(() => {
        async function load() {
            const r = await fetch(`http://90.156.202.71:7000/costsize/type?material_id=${mname.id}`);
    ...
    useEffect(() => {
        async function load() {
            const res = await fetch(`http://90.156.202.71:7000/costsize?id=${mname.id}&size=${pathSize}`);
    ...
    useEffect(() => {
        async function load() {
            const res = await fetch(`http://90.156.202.71:7000/materials?material=${pathItem}`);
    ...
    return (
        <>
             <Layout title={`${mname.material}. Толщина: ${costsize.size} мм. Цена 1 пог. м.: ${costsize.cost} руб | Сивек Ватер Джет Гидроабразив`}>
...
export async function getServerSideProps({query, req}) {
    ...
    const response = await fetch(`http://90.156.202.71:7000/materials?material=${query.material}`);
    const mname = await response.json();

    const res = await fetch(`http://90.156.202.71:7000/costsize?id=${mname.id}&size=${query.size}`);
    const costsize = await res.json();
    //console.log("SERVER Cost ...", costsize, "query size ", query.size, "query id ", mname.id);

    const re = await fetch(`http://90.156.202.71:7000/costsize/type?material_id=${mname.id}`);
