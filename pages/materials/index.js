...
export default function Price({materials:serverMaterials, costsize:serverCostSize}) {
    ...
    const tCostBronze = [];
    for(let i=0; i < costsize.length; i++){
        if(costsize[i].material_id == 28){
            tCostBronze[i] = costsize[i];
        }
    }
    ...
    if(translit == "bronze"){
        const { selectedIndex } = event.target.options;
        const{ id, cost, size} = tCoBronze[selectedIndex];
        setOutPrice(outprice => ({
            ...outprice,
            bronze: {id:id, cost:cost, size:size}
        }))
            ...
            <div className={`${classes.item} ${classes.header}`}>
                ...
                <div className={`${classes.item} ${classes.maincontext}`}>
                     <div>
                        ...
                        <p>Стоимость раскроя материала указана в рублях РФ за один погонный метр <strong className={classes.ndc}>без НДС</strong>. Минимальная стоимость <strong className={classes.ndc}>заказа не должна быть меньше 15000 рублей</strong>.</p>
                        ...
                        <p>Адрес: Посёлок ВУГИ, 1с4, Люберцы, Московская область, 140004. Территория института &laquo;ННЦ ГП - ИГД им. А.А. Скочинского&raquo;</p>
                        <h3>Доставка</h3>
