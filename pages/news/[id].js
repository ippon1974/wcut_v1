...
const Article = ({article:serverArticle}) => {
    ...
    useEffect(()=>{
        async function load() {
            const response = await fetch(`http://90.156.202.71:7000/news?id=${router.query.id}`);
    ...
    return (
        <>
             <Layout title={`${article.title} | Сивек Ватер Джет`} description = {`${article.title} | Гидроабразивная резка`}>
             <Head>
...
export async function getServerSideProps({query, req}) {
    ...
    const response = await fetch(`http://90.156.202.71:7000/news?id=${query.id}`);
