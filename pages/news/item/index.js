...
export default function Item({articles:serverArticles}) {
    ...
    useEffect(()=> {
        async function load() {
            const response = await fetch('http://90.156.202.71:7000/news/item')
    ...
    return(
        <Layout title={'Статьи | Civek Water Jet'} description = {'Статьи. Новости. Гидроабразивная резка'}>
             <Head>
             ...
             <div className={classes.wrapper}>
                ...
                <div className={`${classes.item} ${classes.maincontext}`}>
                    {mobile ? <BlockItemNewsMobile  articles = {articles} /> : <BlockItemNews  articles = {articles} />}
                </div>
...
export async function getServerSideProps({req}) {
    ...
    const res = await fetch('http://90.156.202.71:7000/news/item')
